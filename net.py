...
class Generator(nn.Module):
    def __init__(self, noise=NOISE):
        ...
        self.layer1 = nn.Sequential(
            nn.ConvTranspose2d(noise, 64 * 128, kernel_size=4, stride=1, padding=0, bias=False),
            nn.ReLU(True)
        )
        self.layer2 = nn.Sequential(
            nn.ConvTranspose2d(64 * 128, 64 * 64, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(64 * 64),
            nn.ReLU(True)
        )
        self.layer3 = nn.Sequential(
            nn.ConvTranspose2d(64 * 64, 64 * 32, kernel_size=4, stride=2, padding=1, bias=False),
